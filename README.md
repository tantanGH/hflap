...
PCM8PP.X をあらかじめ常駐させておきます。内蔵ADPCMで再生する場合は PCM8A.X を使用してください。
        ...
        options:
             ...
             -a<n> ... spectrum analyzer mode (0-15,default:6)
...
`-a` オプションを指定するとスペクトラムアナライザを表示します。060turbo実機 + SCSI逐次再生でも48KHzまでのデータであれば間に合う場合がほとんどですが、データによってはバッファアンダーランとなる場合があります。VSYNC割り込みが使用中の場合は使えません。後ろの数字でモードを指定します。

 - bit0 ... バーの色 (0:水色 2:黄色)
 - bit1 ... Lバンドを反転表示するか (0:しない 1:する)
 - bit2 ... ピークホールド表示するか (0:しない 1:する)
 - bit3 ... バンド周波数を表示するか (0:しない 1:する)

デフォルトは6です。再生中に`a`キーを押すと切り替えが可能です。(キーの反応は悪いです)
...
* 0.9.0 (2026/05/02) ... スペアナ表示に対応した
* 0.8.3 (2026/04/18) ... 曲によって再生開始と同時に*が表示される場合があったのを修正
* 0.8.2 (2026/04/04) ... TS16DRVp.X 環境で正常に動作しなくなっていたのを修正
* 0.8.1 (2026/04/04) ... リピート再生時の不具合修正
