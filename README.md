...
FLACデータファイルは直接ハイメモリに読み込むため、060turbo.sysに`-ss`オプションを指定してください。
...
* Raspberry Pi 4B or Rasperry Pi Zero2W
...
68030モードを使っている場合は 030専用バイナリ `HFLAP030.X` の利用を推奨します。

Zero2Wを使う場合、スペクトラムアナライザを使うと処理が間に合わなくなるようです。通常の再生は48kHzまでのデータであればほぼ大丈夫です。

SCSIディスクから読み込む場合はTS16FILE.Xを使用してください。
...
ベースラインJPEGおよびプログレッシブJPEGに対応しています。PNGには対応していません。

WindrvXM/VDISK/HFSドライブ以外のSCSIドライブ等からFLACデータを読み込む場合は TS16FILE.X を活用してください。060turboの場合は060turbo.sysに`-ss`オプションを指定してください。
...
`-A` オプションを指定すると全画面スペクトラムアナライザモードになります。アートワークは表示されません。日本語表示が文字化けします。派手なスペアナのグラフィックを楽しみたい場合などにどうぞ。モードは3種類(0,1,2)だけで色が違うだけです。
...
Win/Mac/Linuxなどで `flac`, `ffmpeg`などのソフトを使って既存のデータから変換することも可能です。この際はあまり重くなりすぎない圧縮にしておくのがポイントです。

アルバムアートを維持しながらApple Losslessでリッピングしたデータをffmpegで変換する推奨コマンドライン例：

```
ffmpeg -i hogehoge.m4a -c:a flac -lpc_type fixed -c:v mjpeg -q:v 2 -f flac hogehoge.fla
```
...
* 1.1.0 (2026/06/16) ... 逐次読み込み処理の改善
* 0.9.9 (2026/06/08) ... 030バイナリは68030専用とした。初期バッファリング中にSHIFTで抜けるとメモリリークしていたのを修正
* 0.9.8 (2026/05/31) ... ハイメモリドライバの存在チェック修正
* 0.9.7 (2026/05/06) ... 全画面スペアナモード(-A)を追加した
* 0.9.6 (2026/05/05) ... スペアナ表示初期化部分の改善
* 0.9.5 (2026/05/04) ... アートワーク表示を高速化。メタデータ読み込み部改善。スペアナのバンド周波数一部変更(100->125)。一部オプション廃止。
